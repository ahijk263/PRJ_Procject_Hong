...
/**
 * EmailService — Gửi email HTML tự động cho Luxury Cars Sử dụng Gmail SMTP
 *
 * CẤU HÌNH: Thay EMAIL_FROM và EMAIL_PASSWORD bằng thông tin thật. Với Gmail
 * cần bật "App Password": Google Account → Security → 2-Step Verification → App
 * passwords
 */
public class EmailService {
    ...
    public static boolean sendOrderConfirmation(String toEmail, String fullName,
        ...
        switch (paymentMethod) {
            case "CASH":
                payLabel = "&#128181; Tiền mặt tại showroom";
                break;
            case "BANK_TRANSFER":
                payLabel = "&#128247; QR Pay / Chuyển khoản";
                break;
            case "INSTALLMENT":
                payLabel = "&#127970; Trả góp ngân hàng";
                break;
            default:
    ...
    // ─────────────────────────────────────────────────────────────
    // 5. TIN NHẮN LIÊN HỆ — gửi đến email web
    // ─────────────────────────────────────────────────────────────
    public static boolean sendContactMessage(String fromName, String fromEmail,
            String phone, String message) {

        String body = h2("Tin nhắn liên hệ mới!")
                + p("Bạn vừa nhận được một tin nhắn liên hệ từ website <strong>Luxury Cars</strong>.")
                + infoTable(new String[][]{
            {"Họ và tên", fromName},
            {"Email", "<a href='mailto:" + fromEmail + "' style='color:#D4AF37;'>" + fromEmail + "</a>"},
            {"Số điện thoại", phone},})
                + divider()
                + "<div style='background:#fafaf7;border-left:4px solid #D4AF37;padding:16px 20px;margin:0;'>"
                + "<p style='margin:0 0 8px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#aaa;'>Nội dung tin nhắn</p>"
                + "<p style='margin:0;font-size:14px;color:#333;line-height:1.7;white-space:pre-wrap;'>" + message + "</p>"
                + "</div>"
                + divider()
                + p("<small style='color:#aaa;'>Để trả lời, hãy reply trực tiếp về email: <strong>" + fromEmail + "</strong></small>");

        return send(EMAIL_FROM,
                "[Luxury Cars] Tin nhắn mới từ " + fromName + " — " + fromEmail,
                wrap("Liên hệ mới", body));
    }

    // ─────────────────────────────────────────────────────────────
    // 6. ADMIN XÁC NHẬN THANH TOÁN / HOÀN THÀNH ĐƠN
    // ─────────────────────────────────────────────────────────────
    public static boolean sendOrderCompleted(String toEmail, String fullName,
            int orderId, String totalPrice, String paymentMethod) {
        ...
        String body = h2("&#127881; Đơn hàng đã hoàn tất!")
                + p("Xin chào <strong>" + fullName + "</strong>,")
                + p("Chúng tôi xin thông báo đơn hàng của bạn đã được <strong style='color:#1a7a4a;'>xác nhận thanh toán</strong> "
                        + "và hoàn tất thành công. Cảm ơn bạn đã tin tưởng Luxury Cars!")
                + infoTable(new String[][]{
            {"Mã đơn hàng", "<strong style='color:#D4AF37;font-size:16px;'>#" + orderId + "</strong>"},
            {"Tổng tiền", "<strong>" + totalPrice + " VNĐ</strong>"},
            {"Thanh toán", payLabel},
            {"Trạng thái", badge("&#10003; Hoàn tất", "#1a7a4a", "#e6f9f0")},})
                + "<div style='background:#e6f9f0;border-left:4px solid #1a7a4a;padding:16px 20px;margin:20px 0;'>"
                + "<p style='margin:0;font-size:13px;color:#1a7a4a;'>"
                + "<strong>&#128663; Bước tiếp theo:</strong> Nhân viên sẽ liên hệ để sắp xếp lịch bàn giao xe. "
                + "Vui lòng giữ điện thoại để không bỏ lỡ cuộc gọi."
                + "</p></div>"
                + btn("Xem chi tiết đơn hàng", "#")
                + divider()
                + "<div style='background:#f9f7f2;border:1px solid #e8e4dc;padding:16px 20px;'>"
        ...
        return send(toEmail,
                "[Luxury Cars] &#127881; Đơn hàng #" + orderId + " đã hoàn tất — Cảm ơn bạn!",
                wrap("Đơn hàng hoàn tất", body));
    }

    // ─────────────────────────────────────────────────────────────
    // 7. ADMIN XÁC NHẬN QR / DUYỆT TRẢ GÓP
    // ─────────────────────────────────────────────────────────────
    public static boolean sendPaymentVerified(String toEmail, String fullName,
            int orderId, String totalPrice, String paymentMethod) {

        String payLabel, payNote;
        switch (paymentMethod) {
            case "BANK_TRANSFER":
                payLabel = "QR Pay / Chuyển khoản";
                payNote = "Giao dịch chuyển khoản của bạn đã được <strong>xác minh thành công</strong>.";
                break;
            case "INSTALLMENT":
                payLabel = "Trả góp ngân hàng";
                payNote = "Hồ sơ trả góp của bạn đã được <strong>duyệt thành công</strong>.";
                break;
            default:
                payLabel = "Tiền mặt";
                payNote = "Thanh toán đã được <strong>xác nhận</strong>.";
        }

        String body = h2("Thanh toán đã được xác nhận!")
                + p("Xin chào <strong>" + fullName + "</strong>,")
                + p(payNote + " Đơn hàng của bạn đang được xử lý.")
                + infoTable(new String[][]{
            {"Mã đơn hàng", "<strong style='color:#D4AF37;font-size:16px;'>#" + orderId + "</strong>"},
            {"Tổng tiền", "<strong>" + totalPrice + " VNĐ</strong>"},
            {"Phương thức", payLabel},
            {"Trạng thái TT", badge("&#10003; Đã xác nhận", "#1a7a4a", "#e6f9f0")},})
                + p("Nhân viên sẽ liên hệ trong thời gian sớm nhất để sắp xếp bàn giao xe.")
                + btn("Xem chi tiết đơn hàng", "#")
                + divider()
                + "<div style='background:#f9f7f2;border:1px solid #e8e4dc;padding:16px 20px;'>"
                + "<p style='margin:0 0 8px;font-size:13px;color:#555;font-weight:700;'>&#128222; Cần hỗ trợ?</p>"
        ...
        return send(toEmail,
                "[Luxury Cars] Xác nhận thanh toán đơn hàng #" + orderId,
                wrap("Thanh toán xác nhận", body));
    }
}
