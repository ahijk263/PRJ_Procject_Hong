...
<html>
    <head>
        <meta charset="UTF-8"><title>Manage Categories - Admin</title>
        <style>
            body{
    ...
    <body>
        <div class="wrapper">
            ...
            <div class="main">
                <div class="topbar">Admin &rsaquo; Catalog &rsaquo; <strong>Categories</strong></div>
                <div class="content">
                    ...
                    <c:if test="${not empty param.error}"><div class="alert alert-error">&#10007; ${param.error}</div></c:if>
                        ...
                        <form method="get" action="${pageContext.request.contextPath}/admin/categories" class="filter-bar">
                        <input type="text" name="keyword" placeholder="Tìm tên category..." value="${keyword}">
                        <button type="submit" class="btn btn-primary">Tìm kiếm</button>
                        <a href="${pageContext.request.contextPath}/admin/categories" class="btn btn-secondary">Reset</a>
                        <button type="button" class="btn btn-success" onclick="document.getElementById('addModal').classList.add('open')" style="margin-left:auto;">+ Thêm Category</button>
                    </form>

                    <div class="card">
                        <div class="card-header">
                            <h3>Danh sách Category</h3>
                            <span style="font-size:13px;color:#888;">Tổng: <strong>${total}</strong></span>
                        </div>
                        <table>
                            <thead><tr><th>ID</th><th>Tên Category</th><th>Mô tả</th><th>Số xe</th><th>Thao tác</th></tr></thead>
                            <tbody>
                                <c:forEach var="c" items="${categories}">
                                    <tr>
                                        ...
                                        <td><span class="badge-count">${carCount[c.categoryId] != null ? carCount[c.categoryId] : 0} xe</span></td>
                                        <td>
                                            <button class="btn btn-warning btn-sm"
                                                    onclick="openEdit(${c.categoryId}, '${c.categoryName}', '${c.description}')">
                                                Sửa
                                            </button>
                                        </td>
                                    </tr>
                                ...
                                <c:if test="${empty categories}">
                                    <tr><td colspan="5" style="text-align:center;padding:24px;color:#aaa;">Không có category nào.</td></tr>
                                </c:if>
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="modal-overlay" id="addModal">
            <div class="modal">
                ...
                <form method="post" action="${pageContext.request.contextPath}/AdminCatalogController">
                    ...
                    <div class="form-group"><label>Tên Category *</label><input type="text" name="categoryName" required placeholder="SUV, Sedan, Coupe..."></div>
        ...
        <div class="modal-overlay" id="editModal">
            <div class="modal">
                ...
                <form method="post" action="${pageContext.request.contextPath}/AdminCatalogController">
                    ...
                    <div class="form-group"><label>Tên Category *</label><input type="text" name="categoryName" id="editCatName" required></div>
                    <div class="form-group"><label>Mô tả</label><textarea name="description" id="editCatDesc"></textarea></div>
                    <div class="modal-footer">
